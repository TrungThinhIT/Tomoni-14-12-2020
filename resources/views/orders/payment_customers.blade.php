                    ...
                    <form action="{{ route('orders.payment-customers.insert') }}" method="POST">
                        ...
                        <fieldset>
                            <div class="form-row" style="margin-left: 2%; margin-top: 1%; margin-right: 1%;">
                                ...
                                <div>
                                    <button type="button" class="btn btn-primary ml-2" onclick="bodyEmpty()"
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <div class="row overflow:auto">
                                        <tr>
                                            <div class="col-3">
                                                ...
                                                <td><input class="form-control" type="text" id="depositID"
                                                        onchange="submitDeposit(this.value)"></td>
                                            ...
                                            <div class="col-3">
                                                ...
                                                <td><input class="form-control" type="date" id="setDate"
                                                        onchange="setDate(this.value)"></td>
                                            </div>
                                    ...
                                    <div style="overflow: auto">
                                        <form id="formSubmit" action="{{ route('orders.payment-customers.addUname') }}"
                                            method="POST">
                                            @csrf
                                            ...
                                            <input class="float-right btn btn-primary" id="checkSubmit" type="submit"
                                                value="Submit">
                                        </form>
                                    </div>
                                </div>
                                            ...
                                            <div>
                                                ...
                                                <script>
                                                    function resetFormSearch() {
                                                        ...
                                                        document.getElementById("Sohoadon2").value = "";
                                                    }

                                                </script>
                                            </div>
                                            <div>
                                                <button id="excel" type="button" class="btn btn-success ml-2">Excel</button>
                                            </div>
                                        </div>
                                    </fieldset>
                                    {{-- modal --}}
                                ...
                                <table class="table table-bordered table-striped" style="margin-top: 1%;">
                                    ...
                                    <tbody id="myTable">
                                        @foreach ($data['PCustomers'] as $item)
                                            ...
                                            @endphp
                                            <tr>
                                                <td data-deposit=@if (count($item) >= 2)  @foreach ($item as $ite)
                                                    {{ $ite->depositID }}
                                                    @break @endforeach
                                                @else
                                                    {{ $item[0]->depositID }}
                                        ...
                                        <td>
                                            <div @if (count($item) >= 2)  @foreach ($item as $ite)
                                                {{ 'id=changeUname' . $ite->depositID }} @endforeach
                                                @endif
                                                >
                                                @if (count($item) >= 2)
                                                    @foreach ($item as $ite)
                                                ...
                                                @else
                                                    <input type="text" class="form-control" id="us{{ $item[0]->Id }}"
                                                        value="{{ $item[0]->uname }}"
                                                        onchange="update{{ $item[0]->Id }}()" placeholder="User name"
                                                        list="litsusername" onkeyup='searchUser(this)'> <datalist
                                                        id='litsusername'></datalist>
                                                @endif
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div @if (count($item) >= 2)  @foreach ($item as $ite)
                                                {{ 'id=changeHoadon' . $ite->depositID }} @endforeach
                                                @endif
                                                >
                                                @if (count($item) >= 2)
                                                    @foreach ($item as $ite)
                                                ...
                                                @else
                                                    <input type="text" class="form-control" id="shd{{ $item[0]->Id }}"
                                                        value="{{ $item[0]->Sohoadon }}"
                                                        onchange="update{{ $item[0]->Id }}()" placeholder="So hoa don"
                                                        list="listbillcode" onkeyup='searchMaHoaDon(this)'> <datalist
                                                        id='listbillcode'></datalist>
                                                @endif
                                            </div>
                                        </td>
                                        <td>
                                            <button class="btn btn-primary shareMoney"
                                                data-sum={{ $sum ?: $item[0]->price_in }} data-deposit="@foreach ($item as $ite) {{ $ite->depositID }}@break @endforeach">Edit</button>
                                        </td>
                                        </tr>
                                        <script>
    ...
    <script>
        ...
        $(document).ready(function() {
            $("#excel").click(function(e) {
                var uname = $("#Uname").val();
                var date_inprice = $("#date_inprice").val();
                ...
                $.ajax({
                    ...
                    type: "GET",
                    url: "/orders/payment-customers/export-excel",
                    data: {
                        Uname: uname,
                        date_inprice: date_inprice,
