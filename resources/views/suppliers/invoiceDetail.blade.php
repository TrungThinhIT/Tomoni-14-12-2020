...
@section('content')
                        ...
                        <div class="form-row">
                            ...
                            <div class="col-md-2 mb-2">
                                ...
                                <input type="number" name="sum" class="form-control"
                                    value="{{ $data['object']->TotalPrice }}" id="sumBillUpdate" placeholder="First name"
                                    onchange="update()">
                            </div>
                            <div class="col-md-2 mb-2">
                            ...
                            <div class="col-md-2 mb-2">
                                ...
                                <select type="text" class="form-control" id="taxBillUpdate" placeholder="First name"
                                    ...
                                    <option value="8" {{ $data['object']->TaxPurchaseCosts == 8 ? 'selected' : '' }}>8%
                                    </option>
                                    <option value="5" {{ $data['object']->TaxPurchaseCosts == 5 ? 'selected' : '' }}>5%
                                    </option>
                                </select>
                            </div>
                            <div class="col-md-2 mb-2">
                        ...
                        <div class="form-row">
                            ...
                            <div class="col-md-2 mb-2">
                                ...
                                <select type="text" class="form-control" name="PaidInvoice" id="PaidInvoiceBillUpdate"
                                    onchange="update()">
                                    <option value="Paid"
                                        {{ $data['object']->InvoiceStatus == 'Paid' ? 'selected' : '' }}>完
                                    </option>
                                    <option value="Unpaid"
                                        {{ $data['object']->InvoiceStatus == 'Unpaid' ? 'selected' : '' }}>未
                                    </option>
                                    <option value="Cancel"
                                        {{ $data['object']->InvoiceStatus == 'Cancel' ? 'selected' : '' }}>消
                                    </option>
                                </select>
                            </div>
                            <div class="col-md-2 mb-2">
                                ...
                                <select type="text" class="form-control" name="Typehoadon" id="TypehoadonBillUpdate"
                                    ...
                                    <option value="Muahang"
                                        {{ $data['object']->TypeInvoice == 'Muahang' ? 'selected' : '' }}>購入
                                    </option>
                                    <option value="TTH" {{ $data['object']->TypeInvoice == 'TTH' ? 'selected' : '' }}>
                                        代行支払
                                    </option>
                                    <option value="Kinhphi"
                                        {{ $data['object']->TypeInvoice == 'Kinhphi' ? 'selected' : '' }}>経費
                                    </option>
                                </select>
                            ...
                            <div class="col-md-2 mb-2">
                                ...
                                <select type="text" class="form-control" name="Buyer" id="BuyerBillUpdate"
                                    ...
                                    <option value="ファム" {{ $data['object']->Buyer == 'ファム' ? 'selected' : '' }}>ファム
                                    </option>
                            ...
                            <tbody>
                                @foreach ($data['object']->detail as $item => $value)
                                    <tr id="{{ $value->Id }}">
                                        <td>
                                            <div>
                                                <input type="text" class="form-control" value="{{ $value->Codeorder }}"
                                                    ...
                                                    onkeyup='search_ordercode(this)'> <datalist
                                                    id='listcodeorder'></datalist>
                                            </div>
                                        </td>
                                        <td>
                                            <div>
                                        ...
                                        <td><button data-id="{{ $value->Id }}" type="button" class="btn btn-danger"
                                                id="delete2" onclick="modalConfirmDelete(this)">Xóa</button></td>
                                    </tr>
                                @endforeach
    ...
    <script>
        ...
        var i = priceDetail + parseInt(cost);
        ...
        function AddMore() {
            ...
            var totalMoney = $("#totalPriceDetail").val();
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response != "false") {
                            ...
                            document.getElementById('totalPriceDetail').value = response
                                .totalPriceInvoiceDetail;
                            ...
                            $('#TableDetaillBill tbody').append("<tr id='" + Id + "'>" +
                                ...
                                "</div>" +
                                "</td>" +
                                "<td><button data-id='" + Id +
                                "' type='button' class='btn btn-danger' onclick='modalConfirmDelete(this)'>Xoá</button></td>" +
                                "</tr>");
                            ...
                            setTimeout(function() {
                                ...
                                if (checkSum > response.totalPriceInvoice) {
                                    document.getElementById("btnAddMore").disabled = false
                                } else {
                                    document.getElementById("btnAddMore").disabled = true
                                }
                            ...
                            toastr.warning('Tổng tiền chi tiết lớn hơn tổng tiền hóa đơn', 'Notifycation', {
                                timeOut: 1000
                            });
                        }
                    },
        ...
        function update() {
            ...
            $.ajax({
                ...
                error: function(response) {
                    $.each(response.responseJSON.errors, function(field_name, error) {
                        $(document).find('[name=' + field_name + ']').after(
                            '<span class="alert-danger-custom" id="' + field_name + '">' + error +
                            '</span>');
                    })
                }
            });
        ...
        function updateDetail(item) {
            ...
            $.ajax({
                ...
                error: function(response) {
                    ...
                    $(document).find('[name=janco_' + id + ']').after(
                        '<span class="alert-danger-custom" id="janco_' + id + '">' + message + '</span>');
                }
            });
        }
        BillDetailCount = 0;
